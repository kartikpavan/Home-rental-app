      ...
      <main className="px-3 md:container mx-auto pt-20 pb-20">
        ...
        <section className="w-full py-5 flex gap-5 flex-wrap">
          {user?.myTrips.map((trip) => {
            const { accomodation, images, name, startDate, endDate, price, tripId, listingId } =
              trip;
            return (
              <Link
                to={`/listing/${listingId}`}
                className="max-w-xl w-full mx-auto bg-white rounded-xl shadow-md overflow-hidden">
